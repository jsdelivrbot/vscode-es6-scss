...
//プルダウン関数
window.addEventListener('load', function () {
    var pulldown = document.querySelectorAll('.pulldown-anime');
    for (var i = 0, length = pulldown.length; i < length; i++) {
...
function Calc() {
    var Result = 0; //価格の合計
    var Num = void 0; //DOMから取得した数値をnumber型に変換する際に一時格納する変数
    var RadioName = []; //class=Calcが付与されたradioのnameを取得
    var count = -1; //RadioName取得でループをする際に使用するカウント変数
    var CalcList = document.querySelectorAll('.Calc');
    for (var i = 0, length = CalcList.length; i < length; i++) {
        CalcList[i].addEventListener('click', function () {
            for (var _i = 0, _length = CalcList.length; _i < _length; _i++) {
                if (CalcList[_i].getAttribute('type') === null) {
                    Num = CalcList[_i].value;
                    Num = Number(Num);
                    Result = Result + Num;
                } else {
                    if (CalcList[_i].checked) {
                        Num = CalcList[_i].value;
                        Num = Number(Num);
                        Result = Result + Num;
                    }
...
///////////////////////////////////////////////////////////
function PullDown() {
    var pulldown = this.id;
    pulldown = document.getElementById(pulldown + 'e');
    var margin = pulldown.style.marginTop;
    //▼ func関数はカウント関数の引数となる
    var func = function x(i) {
...
function FixHeader() {
    var StartPoint = 500; //処理開始位置(num型)
    var StartID = document.getElementById('fixed-header'); //表示したい要素のID
    var StartID2 = document.getElementById('header_title'); //表示したい要素のID2
    clearTimeout(scroll);
